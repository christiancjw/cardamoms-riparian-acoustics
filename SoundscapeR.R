...
multi25_data <- read.csv("clean_data/datasets/indices_datasets/multi25_data.csv")
multi25_1in5_data <- read.csv("clean_data/datasets/indices_datasets/multi25_1in5_data.csv")

single_continuous_rainless_data <- read.csv("clean_data/datasets/indices_datasets/single_continuous_rainless_data.csv")
multi25_rainless_data <- read.csv("clean_data/datasets/indices_datasets/multi25_rainless_data.csv")
multi25_1in5_rainless_data <- read.csv("clean_data/datasets/indices_datasets/multi25_1in5_rainless_data.csv")

global23to25_single_data <- read.csv("clean_data/datasets/indices_datasets/global2023_2025_data.csv")
global23to25_single_rainless_data <- read.csv("clean_data/datasets/indices_datasets/global2023_2025_rainless_data.csv")

songmeters2025 <- read.csv("clean_data/datasets/indices_datasets/songmeters25_data.csv")

### Store datasets in a named list ------------------
datasets <- list(
  "Continuous Single Device" = continuous_single_data,
  "Multi Device 2025" = multi25_1in5_data,
  
  "(RL) Continuous Single Device" = single_continuous_rainless_data,
  "(RL) Multi Device 2025" = multi25_1in5_rainless_data,
  
  "Global Single Device 23-25" = global23to25_single_data,
  "(RL) Global Single Device 23-25" = global23to25_single_rainless_data,
  
  "Songmeters 2025" = songmeters2025
)
...
continuous_single_data <- continuous_single_data %>%
  mutate(Time = as.numeric(Time))
multi25_1in5_data <- multi25_1in5_data %>%
  mutate(Time = as.numeric(Time))

single_continuous_rainless_data <- single_continuous_rainless_data %>%
  mutate(Time = as.numeric(Time))
multi25_1in5_rainless_data <- multi25_1in5_rainless_data %>%
  mutate(Time = as.numeric(Time))

global23to25_single_data <- global23to25_single_data %>%
  mutate(Time = as.numeric(Time))
global23to25_single_rainless_data <- global23to25_single_rainless_data %>%
  mutate(Time = as.numeric(Time))


#### Define available dataframes   -----------------------------
dataframes <- c("Continuous Single Device", "Multi Device 2025",
            "(RL) Continuous Single Device", "(RL) Multi Device 2025", 
            "Global Single Device 23-25", "(RL) Global Single Device 23-25",
            "Songmeters 2025" )
...
site_order <- c("TaCheyHill", "TaChey", "Arai", "Oda", "KnaongBatSa", "TaSay", "Kronomh", "DamFive", "TangRang", "Kravanh Bridge", "PursatTown")
...
#### QBR Colors: continuous gradient red → green -----------------------
  ...
  unique(global23to25_single_data$QBR_Class)
...
output$download_pca <- downloadHandler(
  ...
  content = function(file) {
    ...
    # --- Bind metadata ---
    pca_export <- cbind(
      data %>% select(file_id, Site, Device, Date, Time, Strahler, QBR_Score, Branch, Year, FileName),
      scores
    )
